"use strict";exports.id=1724,exports.ids=[1724],exports.modules={51215:(e,n,t)=>{t.d(n,{L:()=>l});var i=t(75605);let o=process.env.OPENAI_API_KEY;o||console.error("[OpenAI] API key is missing from environment variables!");let a=null;try{a=new i.ZP({apiKey:o})}catch(e){console.error("[OpenAI] Failed to initialize client:",e)}let r=e=>new Promise(n=>setTimeout(n,e));async function s(e,n=0){try{if(!a)throw Error("OpenAI client is not initialized");if(!function(e,n=3e3){return Math.ceil(e.content.length/4)<=n}(e))throw Error("Chunk exceeds token limit");console.log(`[OpenAI] Processing chunk ${e.index} (${e.content.length} chars)`);let n=Date.now(),t=new Promise((e,n)=>{setTimeout(()=>n(Error("Request timeout")),6e4)}),i=a.chat.completions.create({model:"gpt-3.5-turbo",messages:[{role:"system",content:`You are an expert educational content creator, researcher, and teacher with deep knowledge across multiple domains. Create comprehensive, high-quality study notes with exceptional educational value and semantic depth.
...
${e.content}`}],temperature:.7}),o=await Promise.race([i,t]),r=Date.now()-n;return console.log(`[OpenAI] Chunk ${e.index} processed in ${r}ms:`,{promptTokens:o.usage?.prompt_tokens,completionTokens:o.usage?.completion_tokens,totalTokens:o.usage?.total_tokens,model:o.model,responseLength:o.choices[0].message.content?.length||0}),{content:o.choices[0].message.content||"",chunkIndex:e.index}}catch(t){if(console.error(`[OpenAI] Error processing chunk ${e.index} (attempt ${n+1}):`,t),t instanceof Error){let e=t.toString();e.includes("ECONNREFUSED")||e.includes("ETIMEDOUT")||e.includes("network")||e.includes("connection")||e.includes("socket")?console.error("[OpenAI] Network error detected - check internet connection"):e.includes("status code 401")||e.includes("authentication")||e.includes("api key")?console.error("[OpenAI] Authentication error - check API key validity"):e.includes("status code 429")?console.error("[OpenAI] Rate limit exceeded - consider reducing request frequency"):e.includes("status code 400")?console.error("[OpenAI] Bad request error - check input format"):e.includes("timeout")&&console.error("[OpenAI] Request timeout - API call took too long")}if(n<2)return console.log(`[OpenAI] Retrying chunk ${e.index} after ${1e3*(n+1)}ms delay...`),await r(1e3*(n+1)),s(e,n+1);return{content:"",error:t instanceof Error?t.message:"Unknown error",chunkIndex:e.index}}}async function c(e,n=0){try{let n;if(!a)throw Error("OpenAI client is not initialized");console.log(`[OpenAI] Generating quiz for content (${e.length} chars)`);let t=e.length;n=t<=2e3?10:t<=3e3?15:20;let i=Date.now(),o=new Promise((e,n)=>{setTimeout(()=>n(Error("Quiz generation timeout")),6e4)}),r=a.chat.completions.create({model:"gpt-3.5-turbo",messages:[{role:"system",content:`You are an expert educational quiz creator and teacher. Create high-quality multiple-choice questions that test understanding of key concepts and help students learn effectively.
...
${e}`}],temperature:.3}),s=await Promise.race([r,o]),c=Date.now()-i;console.log(`[OpenAI] Quiz generated in ${c}ms`);let l=s.choices[0].message.content||"";try{let e=JSON.parse(l);if(!Array.isArray(e))throw Error("Quiz response is not an array");for(let n of e){if(!n.id||!n.question||!n.options||!n.correctAnswer)throw Error("Invalid question structure");if(!n.options.A||!n.options.B||!n.options.C)throw Error("Missing question options");if(!["A","B","C"].includes(n.correctAnswer))throw Error("Invalid correct answer")}return console.log(`[OpenAI] Successfully generated ${e.length} quiz questions`),{quiz:e}}catch(e){throw console.error("[OpenAI] Failed to parse quiz JSON:",e),console.error("[OpenAI] Raw quiz content:",l),Error("Failed to parse quiz response as valid JSON")}}catch(t){if(console.error(`[OpenAI] Error generating quiz (attempt ${n+1}):`,t),n<2)return console.log(`[OpenAI] Retrying quiz generation after ${1e3*(n+1)}ms delay...`),await r(1e3*(n+1)),c(e,n+1);return{quiz:[],error:t instanceof Error?t.message:"Unknown error generating quiz"}}}async function l(e){console.log(`[OpenAI] Starting notes generation for text (${e.transcript.length} chars)`);let n=Date.now();try{if(!a)throw Error("OpenAI client is not initialized due to configuration issues");if(!e.transcript||"string"!=typeof e.transcript||0===e.transcript.trim().length)throw Error("Empty or invalid transcript provided");let t=function(e,n=800){let t=e.split(/\s+/).filter(e=>e.length>0),i=[];for(let e=0;e<t.length;e+=n){let o=e,a=Math.min(e+n,t.length),r=t.slice(o,a);i.push({content:r.join(" "),index:i.length,wordCount:r.length,startWord:o,endWord:a})}return i}(e.transcript);if(console.log(`[OpenAI] Split text into ${t.length} chunks`),0===t.length)throw Error("No valid text chunks could be created from the transcript");let i=t.map(e=>s(e)),o=await Promise.all(i);o.sort((e,n)=>e.chunkIndex-n.chunkIndex);let r=o.filter(e=>e.error).map(e=>({index:e.chunkIndex,reason:e.error||"Unknown error",attempts:3,startWord:t[e.chunkIndex]?.startWord||0,endWord:t[e.chunkIndex]?.endWord||0})),l=Date.now()-n;if(console.log(`[OpenAI] Notes generation completed in ${l}ms:`,{totalChunks:t.length,successfulChunks:t.length-r.length,failedChunks:r.length}),r.length===t.length)throw Error(`All ${t.length} chunks failed to process. First error: ${r[0]?.reason}`);let p=o.filter(e=>e.content).map(e=>e.content).join("\n\n---\n\n");if(!p||0===p.trim().length)throw Error("No content was generated from any chunks");console.log("[OpenAI] Generating quiz for the notes...");let m=await c(p);m.error?console.warn("[OpenAI] Quiz generation failed:",m.error):console.log(`[OpenAI] Successfully generated ${m.quiz.length} quiz questions`),console.log("[OpenAI] Generating condensed summary from full notes");let d=await u(p);return{content:p,summary:d,quiz:m.quiz,partialSuccess:r.length>0,failedChunks:r.length>0?r:void 0,error:r.length>0?`Niekt\xf3re fragmenty nie zostały przetworzone (${r.length}/${t.length})`:void 0}}catch(i){let e=Date.now()-n;console.error(`[OpenAI] Error generating notes after ${e}ms:`,i);let t="Wystąpił błąd podczas generowania notatek. Proszę spr\xf3bować ponownie.";if(i instanceof Error){let e=i.toString();e.includes("API key")?t="Błąd konfiguracji API. Proszę skontaktować się z administratorem.":e.includes("network")||e.includes("timeout")?t="Problem z połączeniem sieciowym podczas komunikacji z API. Proszę spr\xf3bować ponownie.":e.includes("rate limit")||e.includes("429")?t="Przekroczono limit zapytań do API. Proszę spr\xf3bować ponownie za kilka minut.":(e.includes("empty")||e.includes("invalid transcript"))&&(t="Nie można wygenerować notatek z pustego lub nieprawidłowego transkryptu.")}return{content:"",summary:"",quiz:[],error:t,partialSuccess:!1}}}async function u(e,n=0){try{if(console.log("[OpenAI] Generating condensed summary from full notes"),!a)throw Error("OpenAI client is not initialized");let n=await a.chat.completions.create({model:"gpt-3.5-turbo",messages:[{role:"system",content:`You are an expert educational summarizer specializing in creating concise, high-value summaries of academic content.
...
${e}`}],max_tokens:400,temperature:.3}),t=n.choices[0]?.message?.content?.trim()||"";if(!t)throw Error("Empty summary generated");return console.log(`[OpenAI] Summary generated successfully (${t.length} characters)`),t}catch(t){if(console.error(`[OpenAI] Error generating summary (attempt ${n+1}):`,t.message),n<2)return console.log(`[OpenAI] Retrying summary generation in ${(n+1)*1e3}ms...`),await r((n+1)*1e3),u(e,n+1);return"Nie udało się wygenerować streszczenia."}}},82227:(e,n,t)=>{t.d(n,{DK:()=>r,GV:()=>a});class i{async addToQueue(e,n,t,i,o){let a=this.getPriorityFromPlan(t),r=`${e}_${Date.now()}_${Math.random().toString(36).substr(2,9)}`;return new Promise((t,s)=>{let c={id:r,userId:e,priority:a,requestType:n,payload:i,resolve:async e=>{try{let e=await o();t(e)}catch(e){s(e)}finally{this.processing.delete(r),this.processNext()}},reject:e=>{this.processing.delete(r),s(e),this.processNext()},timestamp:new Date};this.insertByPriority(c),console.log(`[Queue] Added ${n} request for user ${e} with priority ${a}`),this.processNext()})}getQueueStats(e){let n=e?this.getUserPosition(e):null,t=this.queue.reduce((e,n)=>(1===n.priority?e.high++:2===n.priority?e.medium++:e.low++,e),{high:0,medium:0,low:0}),i=n?n*this.averageProcessingTime:this.queue.length*this.averageProcessingTime;return{totalItems:this.queue.length,priorityBreakdown:t,estimatedWaitTime:i}}getUserPosition(e){let n=this.queue.findIndex(n=>n.userId===e);return -1===n?null:n+1}processNext(){if(this.processing.size>=this.maxConcurrent||0===this.queue.length)return;let e=this.queue.shift();e&&(this.processing.set(e.id,!0),console.log(`[Queue] Processing ${e.requestType} for user ${e.userId} (priority ${e.priority})`),setTimeout(()=>{e.resolve(null)},100))}insertByPriority(e){let n=0;for(let t=0;t<this.queue.length;t++){let i=this.queue[t];if(e.priority<i.priority)break;e.priority===i.priority&&e.timestamp>=i.timestamp?n=t+1:e.priority>i.priority&&(n=t+1)}this.queue.splice(n,0,e)}getPriorityFromPlan(e){switch(e){case"pro":return 1;case"student":return 2;default:return 3}}removeFromQueue(e){let n=this.queue.length;return this.queue=this.queue.filter(n=>n.userId!==e),this.queue.length<n}updateAverageProcessingTime(e){this.averageProcessingTime=.8*this.averageProcessingTime+.2*e}getSystemLoad(){let e=this.processing.size,n=this.maxConcurrent;return{queueLength:this.queue.length,processing:e,capacity:n,loadPercentage:e/n*100}}constructor(){this.queue=[],this.processing=new Map,this.maxConcurrent=3,this.averageProcessingTime=90}}let o=new i;async function a(e,n,t,i){return o.addToQueue(e,t,n,{},i)}function r(e){return{stats:o.getQueueStats(e),position:e?o.getUserPosition(e):null,systemLoad:o.getSystemLoad()}}}};